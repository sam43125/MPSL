...
void ADC_Config(void) {
	...
	SET_REG(ADC1->CFGR,ADC_CFGR_RES,01<<3);//00 for 12bit ,01 for 10bit
	//delay_us(200);
	// Prescaler
...
void SystemClock_Config(int reload_val) {
	...
	SET_REG(RCC->CFGR, RCC_CFGR_HPRE, RCC_CFGR_HPRE_DIV4); //0.25MHz
	SET_REG(RCC->CFGR, RCC_CFGR_MCOSEL, 0b011);
	SET_REG(RCC->CFGR, RCC_CFGR_SW, 0b01);
	SET_REG(*(uint32_t *)0xE000E014U, 0xFFFFFF, reload_val);
	SET_REG(*(uint32_t *)0xE000E018U, 0xFFFFFF, 0xFFFFFF); // SysTick current value register (STK_VAL)
...
int main() {
	...
	ADC_Config();
	startADC();
	SystemClock_Config(2000000);
	SET_REG(*(uint32_t *)0xE000E010U, 0x7, 0b110);
	while (1) {
		UART_Transmit("> ", 2);
		i = UART_Receive_Cmd(input, i);
		input[i] = '\0';

		if (!strcmp(input, "showid"))
			UART_Transmit(ID, strlen(ID));
		else if (!strcmp(input, "led on"))
			GPIOA->BSRR = 1 << 5;
		else if (!strcmp(input, "led off"))
			GPIOA->BRR = 1 << 5;
		else if (!strcmp(input, "light")){
			SET_REG(*(uint32_t *)0xE000E010U, 0x7, 0b111);
			while(1){
				while (!(USART1->ISR & USART_ISR_RXNE)); // RXNE: wait until received data is ready to be read
				if(USART1->RDR=='q')
