...
int TableFreq[4][4] = {{382, 340, 303, 0}, {286, 255, 227, 0}, {202, 191, 0, 0}, {0, 0, 0, 0}};
int rows[] = {8,9,10,12};
int cols[] = {5,6,7,9};
#define PRESS 1
#define NOT_PRESS 0
...
void Timer_init(uint32_t reload){
...
int main(){
	...
	int press_stat = NOT_PRESS, befoer_val = -1;
	...
	while(1){
		for(int i=0; i<4; i++) {
			...
			for(int j = 0; j < 4; j++) {
				...
				if(flag_keypad_r != 0) {
					if(TableFreq[j][i]!=0) {
						if(TableFreq[j][i]!=befoer_val) {
							Timer_init(TableFreq[j][i]);
							TIM2->CR1 |= TIM_CR1_CEN;
							befoer_val = TableFreq[j][i];
							press_stat = PRESS;
						}
					}
				}
